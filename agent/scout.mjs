...
function scoutPrompt(repos, avoid = []) {
  ...
  return [
    ...
    "and the definition of done). The agent runs INSIDE the target repo as its working",
    "directory, so write every path RELATIVE to the repo root (e.g. `.github/workflows/ci.yml`)",
    "— NEVER an absolute path like `/Users/.../code/<repo>/...`, and never tell it to `cd`.",
    "Do NOT include any git steps (add/commit/branch/push) or ask it to open a PR: committing",
    "and the pull request are handled automatically after the agent finishes. Set `repo` to the",
    "EXACT folder name from the list above for an existing project, or a short kebab-case name",
    "for a brand-new one.",
    "",
